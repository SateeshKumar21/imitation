...
class SequenceDatasetMultiFile(torch.utils.data.Dataset):
    ...
    def compute_normalization_stats(self, obs_keys_to_normalize):
        ...
        ep = self.demos[0][0]
        obs_traj = {k: self.hdf5_files[0][f"data/{ep}/obs/{k}"][()].astype('float32') for k in obs_keys_to_normalize}
        obs_traj = process_obs_dict(obs_traj, self.obs_keys_to_modality)

        for dataset_key in self.dataset_keys:
            obs_traj[dataset_key] = self.hdf5_files[0][f"data/{ep}/{dataset_key}"][()].astype('float32')
